...
class TabelaVerdade:
    ...
    def atribuicaoValoresTabela(self):
        while len(self.possibilidades) < 2 ** (len(self.conectivos)):
            varTemp = []
            varTemp.append(self.expressao[0].replace(" ", ""))
            for var in self.conectivos:
                varTemp[0] = varTemp[0].replace(var, choice(["V", "F"]))
            varTemp[0] = varTemp[0].replace("~", " ~ ")
            varTemp[0] = varTemp[0].replace("not", " ~ ")
            varTemp[0] = varTemp[0].replace("^", " and ")
            varTemp[0] = varTemp[0].replace("and", " and ")
            varTemp[0] = varTemp[0].replace("or", " or ")
            varTemp[0] = varTemp[0].replace("=>", " => ")
            varTemp[0] = varTemp[0].replace("<=>", " <=> ")
...
class Principal:
    ...
    def montarTabela(self, corColuna=4, corTabela=3):
        ...
        for cont, var in enumerate(self.possibilidades):
            ...
            print(f"{corColuna}|{self.coluna}|{corTabela}{self.resolucao[cont] :^{len(self.expressao[0]) + 7}}{corColuna}|{self.coluna}|{corFinalizar}")
        linha()

x = Principal(["p", "q"], ["p andq orp"])
print(x.montarTabela())
